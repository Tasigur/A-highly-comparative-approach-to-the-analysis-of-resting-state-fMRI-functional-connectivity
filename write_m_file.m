...
for i=1:length(fields1)
    ...
    if flag_isstruct
        ...
        elseif(iscell(this_par))
            % eval(['fprintf(fid,''par.' char(fields1{i}) '=[' eval(['par.' char(fields1{i})]) '];\n'');']);
            % eval(['fprintf(fid,''par.' char(fields1{i}) '=' par.(char(fields1{i})) ';\n'');']);
            eval(['fprintf(fid,''par.' char(fields1{i}) '={'');']);
            for ii=1:length(this_par)
                eval(['fprintf(fid,''''''' par.(char(fields1{i})){ii} ''''''');']);
                if ii==length(this_par)
                    eval(['fprintf(fid,''};\n'');']);
                else
                    eval(['fprintf(fid,'','');']);
                end
...
for i=1:length(stringa_mfile)
    ...
    if nargin_this==1
        ...
        fprintf(fid,[stringa_mfile{i} '(par);\n']);
