function write_qsubscript_slurm_axon(script_name,comm)
...
% fprintf(fid,'#SBATCH --time=84:00:00\n'); % 84 h (3.5 days) suitable for script_See18ReCleanNotSilent*. scripts
% fprintf(fid,'#SBATCH --time=12-00:00:00\n'); % 12 days suitable for long script_See18ReCleanNotSilent*. scripts
fprintf(fid,'#SBATCH --time=30-00:00:00\n'); % 30 days suitable for long script_Smith08ReCleanNotSilent*. scripts
% fprintf(fid,'#SBATCH --time=60-00:00:00\n'); % 60 days to be on the safe side for long script_Form22ReCleanNotSilent*. scripts
...
% fprintf(fid,'#SBATCH --mail-user=Fabiano.Baroni@uam.es\n');
...
% fprintf(fid,'#SBATCH --output=${SLURM_JOB_NAME}_${SLURM_JOBID}.out\n'); % so if the same code is running multiple times out and err files are not overwritten
% fprintf(fid,'#SBATCH --error=${SLURM_JOB_NAME}_${SLURM_JOBID}.err\n');
